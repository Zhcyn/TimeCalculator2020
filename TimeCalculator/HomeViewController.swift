...
class HomeViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, setPickersToValuesDelegate {
    ...
    func createBMIButton() {
        let infoButton = UIButton(type: .custom)
//        infoButton.titleLabel?.text = "BMI"
//        infoButton.titleLabel?.textColor = UIColor.red
        infoButton.setTitle("BMI", for: .normal)
        infoButton.setTitleColor(UIColor.black, for: .normal)
        infoButton.addTarget(self, action: #selector(BMIButtonClicked), for: .touchUpInside)
        let barButton = UIBarButtonItem(customView: infoButton)
    ...
    @objc func BMIButtonClicked(_ sender: UIBarButtonItem!){
        let s = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "CalculateViewController") as! CalculateViewController
        self.present(s, animated: true, completion: {
        })
    }
